...
$result = $koneksi->query("SELECT * FROM booking_test_drive");
?>
  ...
  <tbody>
    <?php $no = 1; while ($row = $result->fetch_assoc()): ?>
      <tr class="hover:bg-gray-100">
        ...
        <td class="p-2 border"><?= htmlspecialchars($row['merk_mobil']) ?></td>
        ...
        <td class="p-2 border space-x-1">
          <?php if ($row['status'] === 'Pending'): ?>
            <a href="setujui_booking.php?id=<?= $row['id_booking'] ?>" class="text-green-600 hover:underline">✔ Setujui</a>
            <a href="tolak_booking.php?id=<?= $row['id_booking'] ?>" class="text-red-600 hover:underline">✖ Tolak</a>
          <?php else: ?>
